...
class ArticleFavoriteController {
    ...
    SingleArticleResponse doPost(RealWorldAuthenticationToken jwt, @PathVariable String slug) {
    ...
    SingleArticleResponse doDelete(RealWorldAuthenticationToken jwt, @PathVariable String slug) {
        var requester = userService.getUser(jwt.userId());
        var article = articleService.getArticle(slug);
